...
static int xive_tctx_post_load(XiveTCTX *tctx, int version_id)
{
    ...
    /*
     * When we collect the states from KVM XIVE device, we set word2
     * of the QEMU XIVE thread context to print out the OS CAM line
     * under the QEMU monitor.
     *
     * This breaks the migration of a TCG guest or a KVM guest not
     * using a KVM XIVE device because the matching algorithm of the
